 ...
 * Description:  This page is used to update a customer record with
 *               the details posted from editCustomerRecord.php
-->
...
<?php 
  ...
  if($customerId==-1 || $customerId==null){
    ...
    if ($stmt = $mysqli->prepare("CALL updateCustomerDetails ((?),(?),(?),(?),(?),(?),(?),(?))")) {
      $stmt->bind_param("isssssss", $customerId, $surname, $fname, $address1, $address2, $postcode, $phoneno, $email);
