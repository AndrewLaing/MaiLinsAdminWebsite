...
--
-- Host: 127.0.0.1
-- Generation Time: Jul 08, 2019 at 12:26 PM
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `getAllThisAfternoonsOrders` ()  BEGIN
SET @today = DATE(NOW());
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `getAllThisMorningsOrders` ()  BEGIN
SET @today = DATE(NOW());
SET @endOfMorning = ADDTIME(CONVERT(@today, DATETIME), '0 12:0:0.0');
SELECT * FROM ordersTBL
WHERE CONVERT(deliveryTimeDate, DATE)=@today
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `getAllTodaysOrders` ()  BEGIN
SET @today = DATE(NOW());
SELECT * FROM ordersTBL
WHERE CONVERT(deliveryTimeDate, DATE)=@today
ORDER BY deliveryTimeDate ASC; 	
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `getAllTomorrowsOrders` ()  BEGIN
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `getAllTonightsOrders` ()  BEGIN
SET @today = DATE(NOW());
SET @startOfTonight = ADDTIME(CONVERT(@today, DATETIME), '0 19:0:0.0');
SELECT * FROM ordersTBL
WHERE CONVERT(deliveryTimeDate, DATE)=@today
...
CREATE TABLE IF NOT EXISTS `customerdetailstbl` (
...
CREATE TABLE IF NOT EXISTS `orderitemstbl` (
...
(3, 'Drumstick', 'Appetisers', 3, 'Battered soy \"chicken\" served with plum sauce.', '4.0000', 'drumstick.jpg'),
...
CREATE TABLE IF NOT EXISTS `staffdetailstbl` (
...
(1, 3, 'Senior Programmer', 'scotty', 'cbfdac6008f9cab4083784cbd1874f76618d2a97', 'Scott', 'Randolph', '13 Pleasant Street', 'Liverpool', 'L19 0NE', '0151345987', 'randyscott@gmail.com'),
...
(3, 1, 'Delivery Manager', 'sallywins', 'b7a875fc1ea228b9061041b7cec4bd3c52ab3ce3', 'Biggins', 'Sally', '22 Plikington Place', 'Liverpool', 'L22 4ES', '08982241161', 'sbiggins@gmail.com'),
...
(5, 2, 'Delivery Staff', 'scottydog', 'bb29a6c01dd5432ccd4d7cdcbefd8b3ecf6d5097', 'Scott', 'Christopher', '6 Alexandra Mews', 'Liverpool', 'L9 0ND', '015109987784', 'chrissyscott@gmail.com');
